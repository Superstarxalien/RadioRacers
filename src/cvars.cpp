...
// Observation Haki
consvar_t cv_applyhaki = Player("radio_hakimode", "Off").values({{0, "Off"}, {1, "On"}}).dont_save().onchange_noinit(KartHaki_OnChange).radio().is_fake_netcvar();
...
consvar_t cv_accessibility_rings_hide = Player("radio_ringsaccessibility", "On").on_off().onchange_noinit(AccessibilityRings_OnChange).radio().is_fake_netcvar();
...
consvar_t cv_battle_toggle_emerald_on_minimap = Player("radio_bttl_emerald_on_minimap", "On").on_off().onchange_noinit(EmeraldsMinimap_OnChange).radio().is_fake_netcvar();
